...
const conditionallyFormatFilePath = (currentFilename, currentImport) => {
  ...
  if (pathSegments.includes('..')) {
    // in relative path
    const proposedAbsolutePath = convertRelativePathToAbsolute(
      currentFilename,
      pathSegments
    );
  ...
  create(context) {
    ...
    return {
      ImportDeclaration(node) {
        const filename = context.eslint
          ? context.eslint.getFilename()
          : context.getFilename();

        const importPath = conditionallyFormatFilePath(
          filename,
          node.source.value
        );

        // react, react-relay/classic
        ...
        const imp = Object.entries(imports)
          .filter(([importString]) => {
            ...
            console.log(importString.includes(importPath));

            return importPath.includes(importString);
          })
